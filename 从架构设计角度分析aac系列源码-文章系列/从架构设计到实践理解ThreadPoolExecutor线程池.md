...
1. [Java线程池实现原理及其在美团业务中的实践](https://tech.meituan.com/2020/04/02/java-pooling-pratice-in-meituan.html):**这篇文章的非常非常有内涵，我的文章也是按照这篇文章思路去解析的，说是转载这篇文章也不为过，区别在于添加了实现代码(画蛇添足)**。

2. [彻底理解Java线程池原理篇](https://cloud.tencent.com/developer/article/1404877)
...
代码实现如下：
	...
	public void execute(Runnable command) {
        ...
        //获取线程池状态和线程池线程数量
        int c = ctl.get();
		//1. 当前线程池中的线程数量少于核心线程数，添加工作线程
        if (workerCountOf(c) < corePoolSize) {
			//添加工作线程方法稍后详细介绍
            if (addWorker(command, true))
                return;
			//如果添加工作线程失败，我们重新获取 线程池状态 + 线程池线程数量
            c = ctl.get();
        }

		//2. 线程池线程数量大于核心线程或者添加工作线程不成功，如果是运行状态，那么添加到等待（阻塞）队列中
        if (isRunning(c) && workQueue.offer(command)) {
            int recheck = ctl.get();
			//3.如果线程池状态是非运行状态，线程移除等待队列，并且执行拒绝策略
...
![](https://p0.meituan.net/travelcube/e9a363c8577f211577e4962e9110cb0226733.png)
...
![](https://p1.meituan.net/travelcube/1a1746f33bfdcb03da074d8539ebb2f367563.png)
...
![](https://p0.meituan.net/travelcube/1df932840b31f41931bb69e16be2932844240.png)
...
![](https://p1.meituan.net/travelcube/668e3c90f4b918bfcead2f4280091e9757284.png)
...
![](https://p0.meituan.net/travelcube/c6caa5be64f39758ada0593b995d65fd25982.png)
...
![](https://p1.meituan.net/travelcube/4d5c410ad23782350cc9f980787151fd54144.png)
...
![](https://p0.meituan.net/travelcube/6c0091e92e90f50f89fd83f3b9eb5472135718.png)

#### 参数动态化

JDK原生线程池ThreadPoolExecutor提供了如下几个public的setter方法，如下图所示：

![](https://p1.meituan.net/travelcube/efd32f1211e9cf0a3ca9d35b0dc5de8588353.png)
...
![](https://p0.meituan.net/travelcube/9379fe1666818237f842138812bf63bd85645.png)
...
![](https://p0.meituan.net/travelcube/414ba7f3abd11e5f805c58635ae10988166121.png)
...
![](https://p0.meituan.net/travelcube/04e73f7186a91d99181e1b5615ce9e4a318600.png)
...
![](https://p1.meituan.net/travelcube/cd0b9445c3c93a866201b7cfb24d2ce7214776.png)

3.运行时状态实时查看

用户基于JDK原生线程池ThreadPoolExecutor提供的几个public的getter方法，可以读取到当前线程池的运行状态以及参数，如下图所示：

![](https://p0.meituan.net/travelcube/aba8d9c09e6f054c7061ddd720a04a26147951.png)

动态化线程池基于这几个接口封装了运行时状态实时查看的功能，用户基于这个功能可以了解线程池的实时状态，比如当前有多少个工作线程，执行了多少个任务，队列中等待的任务数等等。效果如下图所示：

![](https://p1.meituan.net/travelcube/38d5fbeaebd4998f3a30d44bd20b996f113233.png)
