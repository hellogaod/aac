...
class SavedStateRegistry internal constructor() {
     ...
     *
     * 根据key获取当前Bundle数据中的指定参数。并且获取到后删除Bundle中的信息，如果Bundle信息为空，那么近当前Bundle值为null；
     *
     *
     ...
     *
     * 把SavedStateProvider注册到components集合中
     *
     *
     ...
     * to a [Lifecycle].
     *
     *一个生命周期，用于判断当前是否允许存储状态（外部扩展的状态，可外部自定义），onstart表示允许，onstop表示不允许；
     */
    @MainThread
     ...
     *
     * 当执行Activity的onCreate方法时，将当前Activity上一次执行的onSaveInstanceState方法的参数Bundle传递给restoredState变量
     *
     */
    @MainThread
     ...
     *
     * 横竖屏切换activity调用onSaveInstanceState方法时，
     * 将restoredState和components两个变量中的信息存储到onSaveInstanceState方法的参数Bundle中
     *
     * 1.restoredState：横竖屏切换时上一次onSaveInstanceState方法参数保留的Bundle数据
     * 2.components：外部注册进来的SavedStateProvider集合，也是存储viewModel重点
     *
