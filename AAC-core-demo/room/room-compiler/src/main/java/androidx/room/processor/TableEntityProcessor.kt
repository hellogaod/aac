...
class TableEntityProcessor internal constructor(
    ...
    private fun checkIndicesForForeignKeys(
        ...
        //表外键字段指向另外表的字段：要么该字段是主键，要么该字段创建了唯一性索引，否则会报警告；
    ...
    private fun loadSuperIndices(
        ...
        //如果@Entity修饰的当前类的父类也是用了@Entity修饰，那么父类的@Entity#indices属性如果不为空，那么当前类的@Entity#inheritSuperIndices = true，表示一定会将父类中的索引继承；否则报警告；
